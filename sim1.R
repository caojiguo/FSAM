...
alp0 <- c(-1,2)  #coefficient vector for scalar covariates

#FPC score and uniform transformation of FPC score
eigens <- zeta <- matrix(0,nrow=N,ncol=20)
#Monte Carlo 100 simulations
n <- 1000
#MARS, fam, pflr and cosso
size <- matrix(NA, nrow=n, ncol = 4)

#freq for mars, fam, pflr, cosso, cosso1. we choose 20
freq.mars <- freq.fam <- freq.pflr <- freq.cosso <- freq.cosso1 <- rep(0,20)
#mars, fam1, fam, pflr, cosso and cosso1
...
#fam1, fam, pflr and cosso estimate for alpha
alphat.fam1 <- alphat.fam <- alphat.pflr <- alphat.cosso <- alphat.cosso1 <- matrix(NA, ncol=2, nrow=n)
...
{ #s = 10
  ...
  train.data <- data.frame(cbind(y0[train], est.predictor.train, z0[train,]))
  colnames(train.data) <- c("y","pred1", "pred2", "pred4","z1","z2")
  test.data <- data.frame(cbind(y0[test], est.predictor.test, z0[test,]))
  colnames(test.data) <- c("y","pred1", "pred2", "pred4","z1","z2")
  ...
  #M0 <- c(seq(0.2,by=0.25,length.out=10),seq(2.95,by=.5,length.out=4),seq(4.95,by=1,length.out=5))
  # windows()
  ...
  #============================
  cat("cosso with true FPC scores\n")
  
  tun2 <- tune.EMcosso.Gaussian(x=zeta[train,],y=y0[train],z=z0[train,],alp.ini=alp1,
                                wt=rep(1,ncol(zeta[train,])),scale=FALSE,cand.M=M0, tol=0.001, maxit=50,folds=5,plot.it=F)
  fit.cosso <- EM.cosso(x=zeta[train,],y=y0[train],z=z0[train,],alp.ini=alp1,
                        wt=rep(1,ncol(zeta[train,])),scale=FALSE,M=tun2$OptM, tol=0.001, maxit=50)
  alphat.cosso1[s,] <- fit.cosso$alp.est
  fit <- cosso.lin(x=zeta[train,], y=y0[train], z=z0[train,], alp=fit.cosso$alp.est)
  fit.sig <- predict.cosso(fit,M=tun2$OptM,type="nonzero")
  size[s,5] <- length(fit.sig)
  freq.cosso[fit.sig] <- freq.cosso[fit.sig] + 1
  fit.coef <- predict.cosso(fit,M=tun2$OptM,type="coefficients")
  
  cosso.pred <- predict.cosso(fit,xnew=zeta[test,], znew=z0[test,], M=tun2$OptM,type="fit")
  ...
  predictor1.2 <- as.numeric(wsGram(bigGram(zeta[train,], fit$x[fit$basis.id,]), 
                                   theta2/(fit$wt^2))%*%fitObj$coefs)
  predictor1.1 <- as.numeric(wsGram(bigGram(zeta[train,], fit$x[fit$basis.id,]), 
                                   theta1/(fit$wt^2))%*%fitObj$coefs)
  predictor1.4 <- as.numeric(wsGram(bigGram(zeta[train,], fit$x[fit$basis.id,]), 
                                   theta4/(fit$wt^2))%*%fitObj$coefs)
  
  
  
  arg1 <- zeta[train,1][order(zeta[train,1])]
  predictor1.est <- predictor1.1[order(zeta[train,1])]
  predictor1.true <- arg1*exp(arg1)-1
  ise.1 <- trapz(x=arg1, y=(predictor1.est-predictor1.true)^2)
  arg2 <- zeta[train,2][order(zeta[train,2])]
  predictor2.est <- predictor1.2[order(zeta[train,2])]
  predictor2.true <- cos(2*pi*arg2)
  ise.2 <- trapz(x=arg2, y=(predictor2.est-predictor2.true)^2)
  arg4 <-  zeta[train,4][order(zeta[train,4])]
  predictor4.est <- predictor1.4[order(zeta[train,4])]
  predictor4.true <- 3*(arg4-1/4)^2-7/16
  ise.4 <- trapz(x=arg4, y=(predictor4.est-predictor4.true)^2)
